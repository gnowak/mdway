...
export function createNewCard(text, x, y, opts = {}) {
  ...
  const card = { id, text, x, y, w, h, lang, showMd, deckData };
...
function makeCardDOM(card) {
  ...
  const mdBtn = isToggleable
    ? `<button class="cbtn md-btn ${card.showMd ? 'on' : ''}" title="Toggle View Mode">&lang;/&rang;</button>`
    : '';
  ...
  el.innerHTML = `
    <div class="card-hd">
      ...
      ${mdBtn}
      <button class="cbtn export-btn" title="Export Card">💾</button>
      <button class="cbtn edit-btn" title="Edit Card">&#9998;</button>
      <button class="cbtn x-btn" title="Delete Card">&#x2715;</button>
    </div>
...
function renderCardBodyContent(el, card) {
  ...
  if (card.showMd && extensionManager.hasRenderer(card.lang)) {
